...
type BuilderAction =
  ...
  | { type: 'ADD_EXERCISE' }
  | { type: 'UPDATE_EXERCISE'; index: number; payload: Exercise }
  | { type: 'REMOVE_EXERCISE'; index: number }
  | { type: 'SWAP_EXERCISE'; from: number; to: number }
  | { type: 'RESET' };
...
export default function BuilderScreen() {
  ...
  const onSaveCircuit = async () => {
    ...
    setIsLoading(true);
    const circuitToSave: Circuit = {
      id: state.id ?? Date.now().toString(),
      ...state,
      updatedAt: new Date().toISOString(),
    };
    const success = await addCircuit(circuitToSave);
    if (success) {
      await setActiveCircuit(circuitToSave);
...
const styles = StyleSheet.create({
  ...
  previewItem: { color: colors.textSecondary, marginTop: 16 },
